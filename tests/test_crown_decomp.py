...
class TestCrownDecomp(unittest.TestCase):
    ...
    def test_build_bipartite(self):
        """
        Teste la construction d'un sous-graphe biparti à partir des sommets appariés et
        non appariés.
    ...
    def test_crown_decomposition_small(self):
        ...
        Le graphe étant réduit, on s'assure que l'instance n'est pas triviale
        (no_inst=False).
        """
        g = nx.path_graph(3)
    ...
    def test_build_bipartite_empty_matched(self):
        """
        Vérifie la construction d'un sous-graphe biparti lorsque l'ensemble des sommets
        appariés est vide.
    ...
    def test_crown_decomposition_no_crown(self):
        """
        Vérifie le comportement de la décomposition en couronne lorsqu'aucune couronne
        n'est détectée.
