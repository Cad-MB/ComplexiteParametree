...
def benchmark_instance(G, k, edge_density):
    ...
    if not no_inst and ker_g:
        ...
        results.update({
            ...
            "kernel_density": ker_g.number_of_edges() / (ker_g.number_of_nodes() *
                                                         (ker_g.number_of_nodes() - 1) / 2),
    ...
    results.update({
        ...
        "speedup": vcb_time / results["total_ker_time"] if results["total_ker_time"] >
                                                           0 else 0
    })
...
def run_comprehensive_benchmarks(test_configs, edge_probs=None, samples=5):
    ...
    all_results = []
    total_tests = len(test_configs) * len(edge_probs) * samples * 2  # *2 pour random et
    # guaranteed
    current = 0
...
def plot_execution_time(df, output_dir="."):
    """Crée le graphique des temps d'exécution par densité."""
...
def plot_kernel_quality(df, output_dir="."):
    """Crée le graphique de la qualité du kernel."""
    plt.figure(figsize=(10, 6))
...
def plot_density_impact_reduction(df, output_dir="."):
    """Crée le graphique de l'impact de la densité sur la réduction."""
    plt.figure(figsize=(10, 6))
    ...
    for type_g in types:
        means = [df[(df['density'] == d) & (df['type'] == type_g)]['reduction_ratio'].mean()
                 for d in densities]
        plt.plot(densities, means, 'o-', label=type_g)

    plt.xlabel('Densité')
    plt.ylabel('Ratio moyen de réduction')
    plt.title('Impact de la densité sur la réduction')
    plt.legend()
    plt.savefig(f'{output_dir}/density_impact_reduction.png')
    plt.close()


def plot_density_impact_success(df, output_dir="."):
    """Crée le graphique de l'impact de la densité sur le succès."""
    plt.figure(figsize=(10, 6))
    densities = sorted(df['density'].unique())
    types = df['type'].unique()
    ...
    for type_g in types:
        success = [df[(df['density'] == d) & (df['type'] == type_g)]['kernel_success'].mean()
                   for d in densities]
        plt.plot(densities, success, 'o-', label=type_g)

    plt.xlabel('Densité')
    plt.ylabel('Taux de succès')
    plt.title('Impact de la densité sur le succès')
    plt.legend()
    plt.savefig(f'{output_dir}/density_impact_success.png')
    plt.close()


def plot_speedup_distribution(df, output_dir="."):
    """Crée le graphique de la distribution des speedups."""
    plt.figure(figsize=(10, 6))
    densities = sorted(df['density'].unique())
    types = df['type'].unique()
...
def plot_detailed_results(df, output_dir="."):
    """Crée toutes les visualisations séparément."""
    # Créer le répertoire de sortie s'il n'existe pas
    import os
    os.makedirs(output_dir, exist_ok=True)

    # Générer chaque visualisation séparément
    plot_execution_time(df, output_dir)
    plot_kernel_quality(df, output_dir)
    plot_density_impact_reduction(df, output_dir)
    plot_density_impact_success(df, output_dir)
