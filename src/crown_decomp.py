...
def maximal_matching(G: nx.Graph):
    """
    Détermine un couplage maximal dans le graphe G en utilisant une approche
    gloutonne.
...
def crown_decomposition(G: nx.Graph, k: int):
    ...
    - is_no_instance est un booléen indiquant si l'on peut conclure qu'aucune
    couverture de sommets de taille k
    n'existe.
    """
    m = maximal_matching(G)

    # Si le couplage contient plus de k arêtes, alors il est impossible d'obtenir
    # une couverture de taille k
    if len(m) > k:
