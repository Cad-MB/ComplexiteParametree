...
def generate_vertex_cover_graph\
                (n: int, k: int, edge_prob: float = 0.5, guaranteed_vc: bool = False) \
        -> nx.Graph:
    """
    Génère un graphe aléatoire ayant (avec forte probabilité) un vertex cover de taille ≤ k.
    Si `guaranteed_vc` est activé, la structure du graphe est modifiée pour assurer
    l'existence
    ...
    guaranteed_vc : bool, optionnel (par défaut False)
        Si True, assure la construction d'un vertex cover de taille k en forçant les
        connexions.
    ...
    if guaranteed_vc:
        ...
        for u in non_cover:
            ...
            # Ajout d'arêtes supplémentaires aléatoires entre le sommet hors cover et
            # d'autres sommets du cover
            for v in cover:
                if random.random() < edge_prob:
                    g.add_edge(u, v)
    else:
        # Version sans garantie stricte du vertex cover : arêtes générées de manière
        # aléatoire
        list(range(n))
        for i in range(n):
