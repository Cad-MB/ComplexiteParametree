...
def crown_reduction(g: nx.Graph, k: int):
    """
    Applique la réduction par décomposition en couronne de manière itérative
    jusqu'à ce que :
    ...
    tuple (nx.Graph ou None, int, bool)
        ...
        - Un booléen indiquant si l'instance est invalide (aucun vertex cover de
        taille ≤ k).
    """
    g = g.copy()  # Copie pour éviter de modifier l'original
    while True:
        remove_isolated_vertices(g)  # Suppression des sommets isolés
        k = high_degree_rule(g, k)  # Application de la règle des sommets de haut
        # degré
